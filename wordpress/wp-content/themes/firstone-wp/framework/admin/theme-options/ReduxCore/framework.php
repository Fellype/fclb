        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'firstone_domain' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'firstone_domain' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'firstone_domain' );
                }

                if ( empty( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'firstone_domain' );
                }
...
//                    }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'firstone_domain'  text domain
                 */
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        ...
                        if ( true == $this->args['system_info'] ) {
                            add_submenu_page(
                                $this->args['page_slug'],
                                __( 'System Info', 'firstone_domain' ),
                                __( 'System Info', 'firstone_domain' ),
                                $this->args['page_permissions'],
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'firstone_domain' ),
            ...
            public function _enqueue() {
                 ...
                 */
                $save_pending = apply_filters( "redux/{$this->args['opt_name']}/localize/save_pending", __( 'You have changes that are not saved. Would you like to save them now?', 'firstone_domain' ) );
                 ...
                 */
                $reset_all = apply_filters( "redux/{$this->args['opt_name']}/localize/reset", __( 'Are you sure? Resetting will lose all custom values.', 'firstone_domain' ) );
                 ...
                 */
                $reset_section = apply_filters( "redux/{$this->args['opt_name']}/localize/reset_section", __( 'Are you sure? Resetting will lose all custom values in this section.', 'firstone_domain' ) );
                 ...
                 */
                $preset_confirm = apply_filters( "redux/{$this->args['opt_name']}/localize/preset", __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'firstone_domain' ) );
                global $pagenow;
                $this->localize_data['args'] = array(
                    ...
                    'please_wait'           => __( 'Please Wait', 'firstone_domain' ),
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'firstone_domain' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'firstone_domain' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'firstone_domain' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'firstone_domain' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'firstone_domain' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'firstone_domain' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty( $default_output ) ) {
                    $default_output = __( 'Default', 'firstone_domain' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
//                            }
            ...
            public function _options_page_html() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'firstone_domain' ) . '</div></noscript>';
                ...
                if ( ! empty( $this->args['display_name'] ) ) {
                    ...
                    if ( isset( $this->args['dev_mode'] ) && $this->args['dev_mode'] ) {
                        echo '<span class="redux-dev-mode-notice">' . __( 'Developer Mode Enabled', 'firstone_domain' ) . '</span>';
                    }
                ...
                echo '<a href="javascript:void(0);" class="expand_options' . $expanded . '">' . __( 'Expand', 'firstone_domain' ) . '</a>';
                echo '<div class="redux-action_bar">';
                submit_button( __( 'Save Changes', 'firstone_domain' ), 'primary', 'redux_save', false );

                if ( false === $this->args['hide_reset'] ) {
                    echo '&nbsp;';
                    submit_button( __( 'Reset Section', 'firstone_domain' ), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
                    echo '&nbsp;';
                    submit_button( __( 'Reset All', 'firstone_domain' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
                }

                echo '</div>';

                echo '<div class="redux-ajax-loading" alt="' . __( 'Working...', 'firstone_domain' ) . '">&nbsp;</div>';
                echo '<div class="clear"></div>';
                echo '</div>';
                ...
                if ( isset( $this->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->args['opt_name']}", __( 'Settings Imported!', 'firstone_domain' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->args['opt_name']}", __( 'All Defaults Restored!', 'firstone_domain' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->args['opt_name']}", __( 'Section Defaults Restored!', 'firstone_domain' ) ) . '</strong></div>';
                    } else {
                        /**
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'firstone_domain' ) ) . '</strong></div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->args['opt_name']}", __( 'Settings have changed, you should save them!', 'firstone_domain' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . __( 'error(s) were found!', 'firstone_domain' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . __( 'warning(s) were found!', 'firstone_domain' ) . '</strong></div>';
                ...
                if ( $this->args['system_info'] === true ) {
                    ...
                    echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'firstone_domain' ) . '</span></a>';
                    echo '</li>';
                }
                ...
                if ( $this->args['system_info'] === true ) {
                    ...
                    echo '<h3>' . __( 'System Info', 'firstone_domain' ) . '</h3>';
