...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            self::$instance = $this;
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'firstone_domain' ),
                'menu_title'                     => __( 'Install Plugins', 'firstone_domain' ),
                'installing'                     => __( 'Installing Plugin: %s', 'firstone_domain' ),
                'oops'                           => __( 'Something went wrong.', 'firstone_domain' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'firstone_domain' ),
                'dashboard'                      => __( 'Return to the dashboard', 'firstone_domain' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'firstone_domain' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'firstone_domain' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'firstone_domain' ),
                'dismiss'                        => __( 'Dismiss this notice', 'firstone_domain' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        echo '<div id="message" class="error"><p>' . esc_textarea($activate->get_error_message()) . '</p></div>';
                        echo '<p><a href="' . esc_url(add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) )) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . esc_textarea($this->strings['return']) . '</a></p>';
                        return true; // End it here if there is an error with automatic activation
                    }
                    else {
                        echo '<p>' . wp_kses_post($this->strings['plugin_activated']) . '</p>';
                    }
                }
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  wp_kses_post(sprintf( $this->strings['complete']), '<a href="' . esc_url(network_admin_url()) . '" title="' . __( 'Return to the Dashboard', 'firstone_domain' ) . '">' . __( 'Return to the Dashboard', 'firstone_domain' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-activate'] ) && 'activate-plugin' == $_GET['tgmpa-activate'] ) ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    echo '<p><a href="' . esc_url(add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) )) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . wp_kses_post($this->strings['return']) . '</a></p>';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'firstone_domain' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'firstone_domain' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'firstone_domain' );
                }
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'firstone_domain' ) : __( 'Recommended', 'firstone_domain' );
                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'firstone_domain' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'firstone_domain' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'firstone_domain' ) . ' %2$s">' . __( 'Install', 'firstone_domain' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'firstone_domain' ) . ' %2$s">' . __( 'Activate', 'firstone_domain' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'firstone_domain' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        /**
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'firstone_domain' ),
                'source' => __( 'Source', 'firstone_domain' ),
                'type'   => __( 'Type', 'firstone_domain' ),
                'status' => __( 'Status', 'firstone_domain' )
            );
            return $columns;
        }
        /**
        ...
        public function get_bulk_actions() {
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'firstone_domain' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'firstone_domain' ),
            );
            return $actions;
        }
        /**
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'firstone_domain' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                $this->strings['no_package']          = __( 'Install package not available.', 'firstone_domain' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'firstone_domain' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'firstone_domain' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'firstone_domain' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'firstone_domain' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'firstone_domain' );
            }
            /**
            ...
            public function activate_strings() {
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'firstone_domain' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'firstone_domain' );
            }
            /**
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'firstone_domain' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'firstone_domain' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'firstone_domain' ) . '</span><span class="hidden">' . __( 'Hide Details', 'firstone_domain' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'firstone_domain' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'firstone_domain' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'firstone_domain' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'firstone_domain' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'firstone_domain' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'firstone_domain' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'firstone_domain' ) . '</span><span class="hidden">' . __( 'Hide Details', 'firstone_domain' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'firstone_domain' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'firstone_domain' );
                }
            }
            /**
            ...
            public function before( $title = '' ) {
                ...
                echo '<div class="update-messages hide-if-js" id="progress-' . sanitize_html_class( $this->upgrader->update_current ) . '"><p>';
            ...
            public function bulk_footer() {
                ...
                foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . esc_url(add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, network_admin_url( 'themes.php' ) )) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . wp_kses_post(TGM_Plugin_Activation::$instance->strings['return']) . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url(network_admin_url()) . '" title="' . __( 'Return to the Dashboard', 'firstone_domain' ) . '">' . __( 'Return to the Dashboard', 'firstone_domain' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
            }
            /**
