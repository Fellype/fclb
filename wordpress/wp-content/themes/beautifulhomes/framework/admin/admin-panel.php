...
class wt_admin {
	...
	function wt_warnings(){
		...
		/*if ( class_exists('WPBakeryVisualComposerAbstract') && ! defined( 'WT_WPB_VC' )) {
			$warnings[]='You\'re not using <strong><i>WPBakery Visual Composer</i></strong> version that comes with the theme. Please delete your current version and install the bundled one to ensure maximum theme compatibility.';
		}*/
		...
		if(!empty($warnings)){
			...
			$str .= '</ul>';
			echo "
				<div id='theme-warning' class='error fade'><p><strong>".sprintf(esc_html__('%1$s Error Messages','wt_admin'), THEME_NAME)."</strong><br/>".$str."</p></div>
			";
		}
