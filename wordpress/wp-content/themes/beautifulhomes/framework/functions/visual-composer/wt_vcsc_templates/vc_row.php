...
$output .= '<div'.$el_id.' class="wt-row-container'.$row_class.$el_class.$anim_class.'"'.$anim_data.$add_style .'>';
	...
	if ( $bg_type == 'youtube' && $youtube_video_id != '' ) {
		...
		if ( $yt_video_bg_img ) { // if video background image is not empty
			...
			$output .= '<div class="wt_row_bg_video_mobile" style="background-image: url('.$yt_bg_img_url.');"></div>'; 
		}
		
		$output .= '<div class="wt-youtube-bg-wrap wt-youtube-video-'. $id .'">';
		$output .= $overlay; // Overlay for videos
		$output .= '<a id="youtube-bg-video_'. $id .'" class="wt_youtube_player" data-property="{videoURL:\'http://www.youtube.com/watch?v='. $youtube_video_id .'\', containment:\'.wt-youtube-video-'. $id .'\', showControls:'.$yt_controls.', autoPlay:true, loop:true, mute:true, startAt:0, opacity:1, ratio:\'4/3\', addRaster:false, quality:\'default\'}"></a>';
		$output .= '</div>';
	}	
