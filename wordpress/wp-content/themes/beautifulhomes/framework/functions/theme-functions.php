...
add_filter( 'wp_title', 'wt_title', 10, 2 );
...
wpml_register_string( THEME_NAME , 'Logo Text', stripslashes(wt_get_option('general','plain_logo')));
...
function wt_get_excluded_pages(){
	...
	$home = wt_get_option('general','one_page_home');
...
function wt_blog_pagenavi($before = '', $after = '', $blog_query, $paged) {
	...
	$pagenavi_options = array(
		...
		//'next_text' => esc_html__('&raquo;','wt_front'),
		//'prev_text' => esc_html__('&laquo;','wt_front'),
		'next_text' => '<i class="fa-angle-double-right"></i>',
		'prev_text' => '<i class="fa-angle-double-left"></i>',
		'dotright_text' => esc_html__('...','wt_front'),
		'dotleft_text' => esc_html__('...','wt_front'),
		'style' => 1,
		'num_pages' => 4,
		'always_show' => 0,
	...
	if ($max_page > 1 || intval($pagenavi_options['always_show'])) {
		...
		echo balanceTags( $before ) . '<div class="wp-pagenavi">' . "\n";
...
function wt_comment_style($return) {
	return str_replace($return, "<i class=\"fa-comments\"></i>$return", $return);
}
add_filter('get_comment_author_link', 'wt_comment_style');

function wt_span_before_link_list_categories( $list ) {
	$list = str_replace('<a href=','<i class="fa-angle-right"></i><a href=',$list);
	return $list;
}
...
function wt_improved_trim_excerpt($text) {
	global $post;
	if ( '' == $text ) {
...
add_filter('get_the_excerpt', 'wt_improved_trim_excerpt');
...
function wt_new_excerpt_more($output) {
	return $output . ' [...] <a class="wt_read_more" href="'. get_permalink() . '" title="Read more" rel="bookmark">' . esc_html__('Read more','wt_front') . '<i class="fa-angle-double-right"></i></a>';
}
add_filter('get_the_excerpt', 'wt_new_excerpt_more');
...
// remove parentheses from archive list and add span class to post count
...
if (!function_exists('body_classes')) {
	function body_classes($classes) {
		$classes[] = 'wt_onepage_theme';
		if(wt_get_option('general','scroll_to_top')) :
			$classes[] = 'wt-top';
		endif;
		//if(empty(wt_get_option('general','one_page_home'))) :
		if(wt_get_option('general','one_page_home')) :
			$classes[] = 'wt_no_space';
		else :
			$classes[] = 'wt_top_space'; 
		endif;
		...
		if (is_front_page()) {
			...
			if($bgType == 'revSlider') :
				$classes[] = 'wt_rev_bg';
			elseif($bgType == 'image_bg') :
				$classes[] = 'wt_image_bg';
			elseif($bgType == 'parallax') :
				$classes[] = 'wt_parallax';
	    ...
	    if ($args->theme_location == 'primary-menu' && wt_get_option('general','menu_position') == 'side') {
			...
			$logo .=' <li id="menu-item-search" class="menu-item menu-item-logo notMobile">';
				if(!wt_get_option('general','display_logo') && $custom_logo = wt_get_option('general','logo')) {		
                   $logo .='<div id="logo">';
					   if($enable_retina) {
							 $logo .='<a href="'.esc_url( home_url( '/' ) ).'"><img src="'. $custom_logo .'" data-at2x="'. $retinaLogo .'" /></a>';
					   } else {
							 $logo .='<a href="'. esc_url( home_url( '/' ) ) .'"><img src="'. $custom_logo .'" /></a>';
					   }
					$logo .='</div>';
				
				}else {
				   $logo .='<div id="logo_text">';
						 $logo .='<a href="'.esc_url( home_url( '/' ) ).'"><span>'. wt_get_option('general','plain_logo').'</span>';
...
/*function wt_meta() {
 if ( $keys = get_post_custom_keys() ) {
	...
	foreach ( (array) $keys as $key ) {
		$keyt = trim($key);
		if ( '_' == $keyt{0} || 'dfiFeatured' == $keyt || 'textfalse' == $keyt || 'slide_template' == $keyt)
			continue;
...
//	When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
	foreach ( $posts as $post ) {
		...
		if ( empty($post->post_title) )
			$title = $previous ? esc_html__('Previous Post', 'wt_front') : esc_html__('Next Post', 'wt_front');
        ...
        public function process( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
            ...
            if ( ! $dims && ( ( ( null === $height && $orig_w == $width ) xor ( null === $width && $orig_h == $height ) ) xor ( $height == $orig_h && $width == $orig_w ) ) ) {
                ...
                if ( ! $dims || ( true == $crop && false == $upscale && ( $dst_w < $width || $dst_h < $height ) ) ) {
					...
					//whoathemes hack for smaller images
					$img_url = $url;
					$dst_w = $orig_w;
					$dst_h = $orig_h;
                }
...
function wt_get_video($typeurl,$url,$type='full',$layout,$height,$width) {	
    ...
    switch ($typeurl) {
      case 'youtube':	  		
	      $videodiv .= "<iframe class='youtube_video' src='http://{$url}?autohide=1&amp;autoplay=0&amp;controls=1&amp;disablekb=0&amp;fs=1&amp;hd=0&amp;loop=0&amp;rel=0&amp;showinfo=0&amp;showsearch=0&amp;wmode=transparent&amp;enablejsapi=0' width='{$width}' height='{$height}'></iframe>";		
      	  break;
      case 'vimeo':		
		  $videodiv .= "<iframe class='vimeo_video' src='http://player.vimeo.com/video/{$url}?title=0&amp;byline=0&amp;portrait=0&amp;autoplay=0&amp;loop=0' width='{$width}' height='{$height}'></iframe>";
          break;
      case 'dailymotion':
		   $videodiv .= "<iframe class='dailymotion_video' src='http://www.dailymotion.com/embed/video/{$url}?width=$width&amp;autoPlay=0&amp;related=0&amp;chromeless=0&amp;expandVideo=1&amp;theme=none&amp;foreground=%23F7FFFD&amp;highlight=%23FFC300&amp;background=%23171D1B&amp;iframe=1&amp;wmode=transparent' width='{$width}' height='{$height}'></iframe>";
          break;
      case 'metacafe':
...
function wt_media_player($type='full',$layout='',$height='',$width='') {
	...
	$media .= '<iframe width="100%" height="166px" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=' . urlencode($player_link) . '&amp;show_comments=true&amp;auto_play=false"></iframe>';
...
function wt_get_slide($type='full',$layout='',$set_width='',$set_height='') {
	...
	if ($slide_type == "owl") {
		...
		$html_output .= '<div class="wt_slide_holder wt_owl_rotator" data-owl-autoPlay="7000" data-owl-stopOnHover="true" data-owl-navigation="true" data-owl-slideSpeed="800" data-owl-pagination="true" data-owl-pagSpeed="800" data-owl-autoHeight="true"';
		if($type=='left'){
...
/**
 * Get current author page.
 *
 * @return string
 */
if(!function_exists("get_the_author_posts_link")){
	function get_the_author_posts_link(){
		return '<a href="' . get_author_posts_url(get_the_author_meta( 'ID' )) . '" title="' . esc_attr( sprintf( esc_html__('Visit %s&#8217;s all posts','wt_front'), get_the_author()) ) . '" rel="author">' . ucfirst( get_the_author() ) . '</a>';
	}
}
...
function wt_register_required_plugins() {
	$plugins = array(	
		...
		*/
		...
		array(
			'name'     				=> 'Whoathemes Twitter', // The plugin name
			'slug'     				=> 'wt_twitter', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory_uri() . '/framework/plugins/wt_twitter.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
...
load_theme_textdomain('wt_admin');
