...
class wt_themeFeatures {
		...
		if (is_single() || is_page() || (is_front_page() && $post_id != NULL) || (is_home() && $post_id != NULL)){
			...
			if ($type == 'default' && is_singular('post') && $post_id!=$blog_page_id) {
				...
				//if($show_in_header){
					...
					$text .= get_edit_post_link( esc_html__( 'Edit', 'wt_front' ), '<span class="edit-link">', '</span>' );
					if(wt_get_option('blog','single_meta_comment') && (comments_open())){
						ob_start();
						comments_popup_link( esc_html__('No comments ','wt_front'), esc_html__('1 Comment','wt_front'), esc_html__('% Comments','wt_front'),'');
						$text .=  ob_get_clean();
					}
					$text .= '</span>';*/
				//}else{
	...
	function wt_blog_meta() {
		...
		if (wt_get_option('blog','meta_date')){
			$output .= '<div class="entry_date">';
				...
				$output .= '<a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('M d, Y').'</a>';
			$output .= '</div>';
		}
		...
		if (wt_get_option('blog','meta_category')){
			//$output .= '<div class="entry_category"><i class="fa-folder-open"></i></div>';
			$output .= '<span class="wt_meta_separator">/</span>';
	...
	function wt_blog_single_meta() {
		...
		if (wt_get_option('blog','single_meta_date')){
			...
			//$output .= '<i class="fa-calendar-o"></i><a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('d F Y').'</a></div>';
			$output .= '<a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('M d, Y').'</a></div>';
		}
		...
		*/
		...
		if(wt_get_option('blog','single_meta_comment') && ($post->comment_count > 0 || comments_open())){
		    ...
		    //$output .= '<div class="entry_comments"><span>' . esc_html__('Comments: ', 'wt_front') . '</span>';			
			//$output .= '<div class="entry_comments"><i class="fa-comments"></i>';
			$output .= '<span class="wt_meta_separator">/</span>';
			$output .= '<div class="entry_comments">' . ob_get_clean() . '</div>' ;			
		}
