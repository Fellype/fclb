...
class WPBakeryShortCode_WT_testimonial extends WPBakeryShortCode {
	...
	protected function content($atts, $content = null) {
		...
		trim($company) == false ? $company = $company : '';
		...
		$company_string = '<span>'.$company.'</span>';
		...
		$output = '<div id="'.$el_id.'" class="'.$css_class.'"'.$anim_data.'>';
	        ...
	        $output .= "\n\t".'<div class="wt_testimonial_bottom clearfix">';
				...
				$output .= "\n\t\t".'<div class="wt_testimonial_meta">';
					$output .= "\n\t\t\t".'<h4 class="wt_testimonial_author">'.$name_string.$company_string.'</h4>';
				$output .= "\n\t\t".'</div>';
	        $output .= "\n\t".'</div>';
        $output .= '</div>';
...
if (function_exists('wpb_map')) {
	...
	wpb_map( array(
		...
		'params'        => array(
			...
			array(
				'type'          => 'textfield',
				'heading'       => esc_html__('Name', 'wt_vcsc'),
				'holder'        => 'div',
				'param_name'    => 'name',
				'description'   => esc_html__('Set testimonial author name.', 'wt_vcsc')
			),
			array(
				'type'          => 'textfield',
				'heading'       => esc_html__('Company function', 'wt_vcsc'),
				'holder'        => 'div',
				'param_name'    => 'company',
				'description'   => esc_html__('Set testimonial author function.', 'wt_vcsc')
			),
			array(
				'type'          => 'vc_link',
				'heading'       => esc_html__('URL (Link)', 'wt_vcsc'),
				'param_name'    => 'name_link',
				'description'   => esc_html__( 'Select URL if you want the author name to have a link.', 'wt_vcsc' )
			),	
			array(
				'type'          => 'textarea_html',
				'holder'        => 'div',
				'heading'       => esc_html__('Testimonial content', 'wt_vcsc'),
				'param_name'    => 'content',
				'value'         => esc_html__('Here you can enter a small text for this testimonial.', 'wt_vcsc'),
				'description'   => esc_html__('Enter testimonial description.', 'wt_vcsc')
			),	
