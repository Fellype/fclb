			...
			while ($wt_query->have_posts()) : 
				...
				if ($grid == 'true' && $columns != 1) {
					...
					if ($columns == 1) {
						...
						$output .= '<article data-order="'.$i.'" id="post-'.get_the_ID().'" class="blogEntry wt_blog_entry_'.$featured_entry_type.' col-md-4'.$anim_class.' clearfix"'.$anim_data.'>';
					} else {
					...
					if ($featured_entry == 'true') {
						...
						$output .= '<header class="blogEntry_frame">';
						...
						$height = 380;
							...
							switch($thumbnail_type){
								...
								case "timage" : 
									$output .= wt_theme_generator('wt_blog_featured_image','full',$layout,$width='800',$height='600');
									break;
								case "tvideo" : 
								...
								case "tslide" : 
									...
									$output .= wt_get_slide('full',$layout,$width='600',$height='450');
									$output .= '</div>';							
									break;
						}
						$output .= '</header>';
					...
					if($excerpt == 'false'){
						...
						$output .= '<div class="blogEntry_content">';
							...
							if ( $title == 'true' ) {
								$output .= '<h4 class="blogEntry_title">'.get_the_title().'</h4>';
							}
							
							if ( $meta == 'true' ){
								$output .= '<footer class="blogEntry_metadata">';								
								$output .= WT_VCSC_BlogMeta(true, false, false, false, true);							
								$output .= '</footer>';			
							}
							if ( $read_more == 'true' ) {		
