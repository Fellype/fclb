		...
		if ( is_string($thumb_size) && ((!empty($_wp_additional_image_sizes[$thumb_size]) && is_array($_wp_additional_image_sizes[$thumb_size])) || in_array($thumb_size, array('thumbnail', 'thumb', 'medium', 'large', 'full') ) ) ) {
			...
			if (is_array($thumb_size)) {
				...
				if ( $p_img ) {
					...
					$thumbnail = '<img data-florin="flo" class="'.$thumb_class.'" src="'.$p_img['url'].'" width="'.$p_img['width'].'" height="'.$p_img['height'].'" alt="'.$alt.'"'.$thumb_style_str.' />';
					//TODO: APPLY FILTER
				}
			}
		}
...
if ( !function_exists( 'WT_VCSC_Excerpt' ) ) {
	function WT_VCSC_Excerpt( $length=30, $readmore, $read_more_text, $post_id='' ) {
		global $post;
		...
		if ( '0' != $length ) {
			...
			if ( $custom_excerpt ) {
				...
				// Readmore text
				$read_more_text = $read_more_text ? $read_more_text : esc_html__('View Post', 'wt_vcsc' );
...
if ( !function_exists( 'WT_VCSC_BlogPageNavi' ) ) {
	function WT_VCSC_BlogPageNavi($before = '', $after = '', $blog_query, $paged) {
		...
		$pagenavi_options = array(
			//'pages_text' => esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%','wt_front'),
			// 'pages_text' => '',
...
// Get the author archive page - duplicated
...
if ( !function_exists( 'WT_VCSC_BlogMeta' ) ) {
	function WT_VCSC_BlogMeta($date=false, $author=false, $category=false, $tags=false, $comments=false) {
 		global $post;
		$output = '';
		if ( $date ) {
			$output .= '<div class="entry_date">';
				//$output .= '<i class="fa-calendar-o"></i><a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('d F Y').'</a></div>';
				$output .= '<a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F d').', '.get_the_time('Y').'</a>';
			$output .= '</div>';
		}
		if ( $author ) {
			...
			$output .= '<div class="entry_author">';
				...
				if (class_exists('wt_themeFeatures')) {
				...
				$output .= '<span>' . esc_html__('By ', 'wt_front') . '<span class="wt_author_link">' . $author . '</span></span>';
				
			$output .= '</div>';
		}
		if ( $category ) {
			//$output .= '<div class="entry_category"><i class="fa-folder-open"></i>';
			$output .= '<span class="wt_meta_separator">/</span>';
			$output .= '<div class="entry_category">';
				$output .= '<span>' . esc_html__('In: ', 'wt_front') . '</span>';
		...
		if ( $tags ) {
			...
			$output .= '<span class="wt_meta_separator">/</span>';
			$output .= get_the_tag_list('<div class="entry_tags"><span>' . esc_html__('Tags ', 'wt_front') . '</span>',', ','</div>'); 
		}
		
		$output .= get_edit_post_link( esc_html__( 'Edit', 'wt_front' ), '<span class="edit-link">', '</span>' );
		if( $comments && ($post->comment_count > 0 || comments_open())){
			ob_start();
