...
"type" => "internal",		"active" => "true"),
			...
			"WT Pie Chart"					        => array("setting" => "PieChart",                 "file" => "pie_chart",              "type" => "internal",		"active" => "true"),
...
"type" => "internal",		"active" => "true"),
		...
		function wt_multidropdown_settings_field($settings, $value) {	
			...
			$output .= '<select name="'.$settings['param_name'].'" class="wpb_vc_param_value wpb-input wpb-select ' . $param_name . ' ' . $type . '" data-option="'.$css_option.'"  multiple ' . $dependency . '>';
				...
				foreach ( $values as $text_val => $val ) {
					...
					$text_val = htmlspecialchars( esc_html__($text_val, "wt_vcsc") );					
		...
		function wt_markers_settings_field($settings, $value) {
			...
			<script type="text/javascript">
				jQuery(document).ready(function( $ ) {
					...
					var textarea_val = $('#vc_properties-panel .marker_data').html();
					if(textarea_val != '') {
		...
		function WT_VCSC_AddParametersToComposer() {
			if (function_exists('add_shortcode_param')) {
				...
				add_shortcode_param('wt_separator',        	array($this, 'wt_separator_settings_field'));
				
				// Generate param type "wt_multidropdown"
				add_shortcode_param('wt_multidropdown',     array($this, 'wt_multidropdown_settings_field'));
				
				// Generate param type "wt_range"
				add_shortcode_param('wt_range',             array($this, 'wt_range_settings_field'));
								
				// Generate param type "wt_loadfile"
				add_shortcode_param('wt_loadfile',          array($this, 'wt_loadfile_settings_field'));
				
				// Generate param type "wt_markers"
				add_shortcode_param('wt_gmap_markers',      array($this, 'wt_markers_settings_field'));
			}
		}
		// ! Load Extended Composer Elements
		// --------------------------------------------
		...
		function WT_VCSC_Extensions_Front() {
			...
			if (!empty($post)){
				...
				//wp_register_style( 'wt-extend-youtube-player',                       $url . 'assets/lib/css/YTPlayer.css',                  null, false, 'all' );
				wp_register_style( 'wt-extend-youtube-player',                       THEME_VC_CSS . '/YTPlayer.css',                        null, false, 'all' );
				...
				//wp_register_script( 'wt-extend-easypiechart',                        $url . 'assets/lib/js/jquery.easypiechart.min.js',    array('jquery'), null, true );
				wp_register_script( 'wt-extend-easypiechart',                        THEME_VC_JS . '/jquery.easypiechart.min.js',          array('jquery'), null, true );	
		...
		function WT_VCSC_Extensions_Css_Classes($wt_class, $tag) {
			if ( in_array( $tag, array('vc_tabs', 'vc_accordion', 'vc_toggle', 'vc_pie', 'TS-VCSC-Pricing-Table', 'TS-VCSC-Countdown') ) ) {
