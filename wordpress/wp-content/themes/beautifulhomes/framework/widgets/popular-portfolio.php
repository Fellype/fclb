...
class Wt_Widget_Popular_Portfolio_Posts extends WP_Widget {
	...
	function Wt_Widget_Popular_Portfolio_Posts() {
		$widget_ops = array('classname' => 'widget_popular_portfolio_posts', 'description' => esc_html__( "The most popular portfolio posts", 'wt_admin') );
		$this->WP_Widget('popular_portfolio_posts', THEME_SLUG.' - '.esc_html__('Popular Portfolio Posts', 'wt_admin'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Popular Portfolio Posts', 'randf_front') : $instance['title'], $instance, $this->id_base);
...
<?php elseif($display_extra_type == 'comments'):?>
					<span class="comments"><?php echo comments_popup_link(esc_html__('No response ','randf_front'), esc_html__('1 Comment','randf_front'), esc_html__('% Comments','randf_front'),''); ?></span>
