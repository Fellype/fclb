...
class Wt_Widget_Related_Posts extends WP_Widget {
	...
	function Wt_Widget_Related_Posts() {
		$widget_ops = array('classname' => 'widget_related_posts', 'description' => esc_html__( "The most related posts on your site", 'wt_admin') );
		$this->WP_Widget('related_posts', THEME_SLUG.' - '.esc_html__('Related Posts', 'wt_admin'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Related Posts', 'wt_front') : $instance['title'], $instance, $this->id_base);
...
<?php if($show_title || $show_time || $show_description || $show_comments):?>
	...
	<?php if($show_description):?><p><?php echo wp_html_excerpt(get_the_excerpt(),$desc_length);?></p><?php endif; //end display extra description ?>
	<?php if($show_comments):?><span class="comments"><?php echo comments_popup_link(esc_html__('No response ','wt_front'), esc_html__('1 Comment','wt_front'), esc_html__('% Comments','wt_front'),''); ?></span><?php endif;//end display extra comments ?>
				</div>
